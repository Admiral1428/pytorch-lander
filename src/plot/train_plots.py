...
def plot_results(path):
    ...
    with PdfPages(path + ".pdf") as pdf:
        pdf.savefig(plot_rewards(df))
        pdf.savefig(plot_event_rate(df))
        pdf.savefig(plot_vertical_velocity(df))
        pdf.savefig(plot_vertical_distance_to_pad(df))
        pdf.savefig(plot_horizontal_distance_to_pad(df))
        pdf.savefig(plot_action_mix(df))
        pdf.savefig(plot_exploration(df))
        pdf.savefig(plot_reward_components(df))
        pdf.savefig(plot_terminal_vs_shaping(df))
...
def plot_vertical_distance_to_pad(df):
    plt.figure(figsize=(12, 4))
...
def plot_horizontal_distance_to_pad(df):
    plt.figure(figsize=(12, 4))
...
def plot_terminal_vs_shaping(df):
    shaping_total = (
